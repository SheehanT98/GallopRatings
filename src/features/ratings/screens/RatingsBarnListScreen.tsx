  ...
  return (
    <ScreenContainer>
      <SectionCard>
          ...
          <View className="flex-row flex-wrap gap-2">
            {(locationsQuery.data ?? []).map((location) => {
              ...
              return (
                <Pressable
                  key={location.id}
                  onPress={() => setLocationId((prev) => (prev === location.id ? undefined : location.id))}
                  className={`rounded-full px-3 py-2 ${
                    isSelected ? 'bg-brand-600' : 'bg-slate-200'
                  }`}
                >
                  <Text className={isSelected ? 'text-white' : 'text-slate-700'}>{location.name}</Text>
                </Pressable>
              );
            })}
          </View>
        ...
        <View className="mt-3">
          <Field label="Horse search" value={search} onChangeText={setSearch} placeholder="Search horse..." />
        </View>
      </SectionCard>
